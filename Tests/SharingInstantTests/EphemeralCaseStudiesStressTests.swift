...
final class EphemeralCaseStudiesStressTests: XCTestCase {
  ...
  override func setUp() async throws {
    ...
    self.clientInstanceID = "case-studies-stress-\(UUID().uuidString.lowercased())"
    self.store = SharedTripleStore()
    self.reactor = Reactor(store: self.store, clientInstanceID: self.clientInstanceID)

    await MainActor.run {
      InstantClientFactory.clearCache()
    }

    self.client = InstantClientFactory.makeClient(appID: self.app.id, instanceID: self.clientInstanceID)
    try await InstantTestAuth.signInAsGuestAndReconnect(client: self.client, timeout: 15)
  }
  ...
  func testTodoStressMultipleCreatesAndTogglesRoundTrip() async throws {
    ...
    let instanceID = self.clientInstanceID!
    ...
    try await withDependencies {
      ...
      try await Self.eventually(
        ...
        toggledIds.allSatisfy { id in
          let decoded: Todo? = store.get(id: id)
    ...
    try await withDependencies {
      ...
      try await Self.eventually(
        ...
        postIds.allSatisfy { id in
          let decoded: Post? = store.get(id: id)
      ...
      try await Self.eventually(
        ...
        let decoded: Post? = store.get(id: postIdToSwitch)
